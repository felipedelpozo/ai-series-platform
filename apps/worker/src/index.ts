...
import { checkDb, getDb, type DbHealth } from "@ai-series/db";
...
export function run(env: AppEnvConfig): void {
  ...
  try {
    void runWorkerLoop(getDb());
  } catch (error) {
    console.warn("[worker] job loop disabled:", error instanceof Error ? error.message : error);
  }
}
